...
if __name__ == "__main__":
    for test_people, train_people in zip([["joel"], ["manuel"], ["pedro"]], [["manuel", "pedro"], ["joel", "pedro"], ["joel", "manuel"]]):
        ...
        for _ in range(50):
            # read data
            x_train, y_train = read_dataset2(people=train_people)
            x_test, y_test = read_dataset2(people=test_people, num_samples=3663)
            ...
            #                         show=False, save_path = f"./results/cnn_2vs1_{test_people[0]}_acc_comparison.svg")

            # plot_loss_comparison([results.history["loss"], results.history["val_loss"]],
            #                     "Training/Validation Loss Comparison",
            #                     ["Training Loss", "Validation Loss"],
            #                     show=False, save_path = f"./results/cnn_2vs1_{test_people[0]}_loss_comparison.svg")

            # plot_confusion_matrix(y_test, y_pred, ["bottle", "cube", "phone", "screwdriver"],
            #                     show=False, save_path = f"./results/cnn_2vs1_{test_people[0]}_conf_matrix.svg")
            ...
            #             save_path = f"./results/cnn_2vs1_{test_people[0]}_results.txt")

        f = open(f"./results/cnn_inter_user_{test_people[0]}_results.txt", "w")
